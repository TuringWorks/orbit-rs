-- End-to-end banking example using SQL, ML, vectors, and GraphRAG
-- Sections:
-- 1) Data model and sample data
-- 2) Time-window analytics
-- 3) Fraud features, model training, and prediction
-- 4) Credit scoring features and prediction
-- 5) GraphRAG knowledge ingestion and policy reasoning
...
-- Seed customers, accounts, merchants, and a few transactions
...
-- Rolling 60-minute sum and count per account for recent transactions
WITH ts_win AS (
...
-- Train a random forest fraud model on engineered features
...
-- Feature engineering for credit scoring across customers
...
-- Predict credit score-like ranking per customer
SELECT customer_id,
...
-- Ingest policy documents into GraphRAG knowledge graph
...
-- Query the graph with RAG to explain applicable rules
