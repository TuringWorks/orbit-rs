...
def get_connection():
    ...
    try:
        return psycopg2.connect(
            ...
            dbname=PG_DBNAME,
            # sslmode='require' # Orbit might not support SSL yet
        )
    except OperationalError as e:
        print(f"Connection failed: {e}")
        sys.exit(1)

# Define the features to test (from PCI)
FEATURES = {
    ...
    try:
        if feature_category == "data_types":
            if feature_name == "Primitive Types":
                ...
                cursor.execute("CREATE EXTENSION IF NOT EXISTS postgis; CREATE TABLE test_geo (geom GEOMETRY);")
                ...
                cursor.execute("CREATE EXTENSION IF NOT EXISTS vector; CREATE TABLE test_vector (embedding VECTOR(3));")
            ...
            if feature_name == "Extension Support":
                ...
                return "full" if extensions != 0 else "no"
        ...
        return "full"
    except (errors.SyntaxError, errors.UndefinedFunction, errors.FeatureNotSupported, Exception) as e:
        # print(f"Feature {feature_name} failed in {feature_category}: {e}")
        return support

def calculate_pci(features):
    """Calculate the PCI score."""
    total_score = 0
    penalty = 0
    failed_tests = []
...
def main():
    ...
    for category, subfeatures in FEATURES.items():
        for subfeature in subfeatures:
            print(f"Testing {category} -> {subfeature}...", end="\r")
