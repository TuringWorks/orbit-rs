...
impl LsmTreePersistence {
    ...
    async fn insert_internal(&self, key: ActorKey, value: Option<ActorLease>) -> Result<PersistenceMetrics, PersistenceError> {
        ...
        // Log to WAL first
        {
            ...
            wal.buffer.extend_from_slice(&(serialized.len() as u32).to_le_bytes());
    ...
    async fn rotate_memtable(&self) -> Result<(), PersistenceError> {
        let old_memtable = {
            let mut memtable = self.memtable.write().unwrap();
            let old = std::mem::replace(&mut *memtable, MemTable::new());
            old
        };
    ...
    async fn generate_sstable_path(&self, level: usize) -> PathBuf {
        ...
        sstable_dir.join(format!("level_{}_{}_{}.sst", level, timestamp, rand::random::<u32>()))
    }
    
    async fn write_sstable(&self, memtable: &MemTable, path: &Path) -> Result<(), PersistenceError> {
        ...
        for (key, value) in &memtable.data {
            ...
            writer.write_all(&(serialized.len() as u32).to_le_bytes()).await?;
    ...
    fn build_bloom_filter(&self, memtable: &MemTable) -> BloomFilter {
        ...
        let num_hash_functions = ((self.config.bloom_filter_bits_per_key as f64) * (2.0_f64.ln())).ceil() as usize;
    ...
    async fn compact_level(&self, level_index: usize) -> Result<(), PersistenceError> {
        ...
        if !merged_data.is_empty() {
            ...
            if next_level < self.config.max_levels {
                ...
                self.write_merged_sstable(&merged_data, &new_sstable_path).await?;
    ...
    async fn read_sstable(&self, path: &Path) -> Result<BTreeMap<ActorKey, Option<ActorLease>>, PersistenceError> {
    ...
    async fn write_merged_sstable(&self, data: &BTreeMap<ActorKey, Option<ActorLease>>, path: &Path) -> Result<(), PersistenceError> {
        let file = File::create(path).await?;
        let mut writer = BufWriter::new(file);
    ...
    async fn search_sstables(&self, key: &ActorKey) -> Result<Option<ActorLease>, PersistenceError> {
    ...
    async fn search_in_sstable(&self, path: &Path, key: &ActorKey) -> Result<Option<Option<ActorLease>>, PersistenceError> {
...
impl PersistenceProvider for LsmTreePersistence {
    async fn store_lease(&self, lease: &ActorLease) -> Result<PersistenceMetrics, PersistenceError> {
        self.insert_internal(lease.key.clone(), Some(lease.clone())).await
    }
    ...
    async fn create_snapshot(&self) -> Result<(String, PersistenceMetrics), PersistenceError> {
        let start_time = Instant::now();
        let snapshot_id = format!("lsm_snapshot_{}", 
...
mod tests {
    ...
    async fn test_lsm_basic_operations() {
        ...
        let lsm = LsmTreePersistence::new(config, temp_dir.path()).await.unwrap();
    ...
    async fn test_lsm_memtable_flush() {
        ...
        config.memtable_size_mb = 1; // Small memtable for testing
