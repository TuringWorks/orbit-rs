...
async fn demo_file_config(base_dir: &Path, test_leases: &[ActorLease]) -> Result<(), PersistenceError> {
    println!("Testing file-based configuration...");
    
    // Create configurations for different backends
    let configs = vec![
        ("cow_config.json", PersistenceConfigBuilder::new()
        ...
        ("lsm_config.json", PersistenceConfigBuilder::new()
    ...
    for (filename, config) in configs {
        ...
        println!("  ✓ Data verified for {}: {}", filename, retrieved.is_some());
    }
    
    Ok(())
}

async fn demo_builder_config(base_dir: &Path, test_leases: &[ActorLease]) -> Result<(), PersistenceError> {
    ...
    println!("    Write: {:?} | Read: {:?} | Success: {}", 
             cow_write.latency, cow_read.latency, cow_read_result.is_some());
    ...
    println!("    Write: {:?} | Read: {:?} | Success: {}", 
             lsm_write.latency, lsm_read.latency, lsm_read_result.is_some());
    ...
    println!("    Write: {:?} | Read: {:?} | Success: {}", 
             rocks_write.latency, rocks_read.latency, rocks_read_result.is_some());
    
    Ok(())
}

async fn demo_performance_comparison(base_dir: &Path, test_leases: &[ActorLease]) -> Result<(), PersistenceError> {
    ...
    for (name, provider) in providers {
        ...
        println!("{:<12} {:<12} {:<10.1}μs {:<12} {:<10.1}μs", 
                 name, write_times.len(), write_avg, read_times.len(), read_avg);
    }
