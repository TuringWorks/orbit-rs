...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    
    {
        ...
        for i in 0..10 {
            ...
            println!("   📝 Stored lease {} in {:.1}μs", i, metrics.latency.as_micros());
        ...
        println!("   📊 Stats: {} keys, {} bytes in memory", stats.total_keys, stats.memory_usage_bytes);
    ...
    if wal_path.exists() {
        ...
        if wal_content.len() > 0 {
            println!("   📄 WAL file contains {} bytes of data", wal_content.len());
    ...
    
    {
        ...
        let stats = persistence.get_stats().await?;
        println!("   📊 After recovery: {} keys, {} bytes in memory", stats.total_keys, stats.memory_usage_bytes);
        ...
        println!("   📈 Recovery Summary: {}/{} leases recovered", recovered_count, initial_leases.len());
        ...
        if recovered_count == 0 {
            ...
            println!("   💡 This is expected for a prototype - full recovery needs to be implemented");
    ...
    if wal_path.exists() {
        ...
        while offset + 4 <= wal_content.len() {
            ...
            let entry_len = u32::from_le_bytes([len_bytes[0], len_bytes[1], len_bytes[2], len_bytes[3]]) as usize;
            offset += 4;
            ...
            if offset + entry_len <= wal_content.len() {
                ...
                match serde_json::from_slice::<serde_json::Value>(entry_data) {
                    Ok(entry_json) => {
                        ...
                        if let Some(op_type) = entry_json.get("operation").and_then(|op| op.get("type")) {
