...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   ✅ Stored 1000 actors in {:.2}ms", batch_time.as_millis());
    println!("   📊 Average: {:.1}μs per actor", batch_time.as_micros() as f64 / 1000.0);
    println!("   🚀 Throughput: {:.0} ops/sec", 1000.0 / batch_time.as_secs_f64());
    ...
    println!("   📊 Average renewal time: {:.1}μs", avg_renewal.as_micros());
    ...
    println!("   💾 WAL replay data: {} bytes", recovery_metrics.disk_bytes_read);
    ...
    for key in &actor_keys[..100] { // Check first 100
    ...
    println!("   ✅ Verified 100 actors in {:.2}ms", verification_time.as_millis());
