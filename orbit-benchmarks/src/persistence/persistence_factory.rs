use super::config::{PersistenceBackend, PersistenceConfig};
    ...
    pub async fn create_provider(
        ...
        match config.backend {
            ...
            PersistenceBackend::LsmTree => {
                let lsm_persistence =
                    LsmTreePersistence::new(config.lsm_config.clone(), &config.data_dir).await?;
    ...
    pub async fn create_all_providers(
        ...
        // RocksDB
        {
            ...
            let rocksdb_provider =
                Self::create_default(PersistenceBackend::RocksDb, &rocksdb_dir).await?;
...
mod tests {
    use super::*;
    use crate::persistence::ActorLease;
    ...
    fn test_backend_characteristics() {
        let (write_latency, read_latency) =
            PersistenceFactory::expected_performance(&PersistenceBackend::CowBTree);
