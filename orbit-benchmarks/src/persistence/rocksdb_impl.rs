...
use rocksdb::{ColumnFamily, ColumnFamilyDescriptor, Options, ReadOptions, WriteOptions, DB};
...
impl RocksDBPersistence {
    pub async fn new(data_dir: &std::path::Path) -> Result<Self, PersistenceError> {
        ...
        db.cf_handle("leases").ok_or_else(|| {
            PersistenceError::Corruption("Missing leases column family".to_string())
        })?;

        db.cf_handle("cluster").ok_or_else(|| {
            PersistenceError::Corruption("Missing cluster column family".to_string())
        })?;
...
impl PersistenceProvider for RocksDBPersistence {
    async fn store_lease(
        ...
        let result = self
            .db
            .put_cf_opt(cf, &key, &value, &self.lease_write_options);
        ...
        match result {
            ...
            Err(e) => Err(PersistenceError::Io(std::io::Error::other(e))),
        }
    }
    ...
    async fn range_query(
        ...
        for item in iter {
            match item {
                ...
                Err(e) => return Err(PersistenceError::Io(std::io::Error::other(e))),
            }
        }
...
mod tests {
    ...
    async fn test_rocksdb_basic_operations() {
        ...
        let (range_results, range_metrics) =
            persistence.range_query(&lease.key, &end_key).await.unwrap();
