...
use orbit_benchmarks::persistence::metrics::{BenchmarkResults, MetricsAnalyzer};
use orbit_benchmarks::persistence::rocksdb_impl::RocksDBPersistence;
use orbit_benchmarks::persistence::workload::{CrashSimulator, WorkloadSimulator};
...
async fn run_comprehensive_benchmark<P: PersistenceProvider>(
    ...
    println!(
        "Running actor lease workload for {}...",
        implementation_name
    );
    ...
    println!(
        "Running high throughput workload for {}...",
        implementation_name
    );
    ...
    println!(
        "Running query heavy workload for {}...",
        implementation_name
    );
...
fn benchmark_comprehensive_workloads(c: &mut Criterion) {
    ...
    for impl_result in &analysis.implementations {
        ...
        println!(
            "- **Write Latency P95**: {:.1}μs",
            impl_result
                .actor_lease_workload
                .write_latency_p95
                .as_micros()
        );
        println!(
            "- **Read Latency P95**: {:.1}μs",
            impl_result
                .actor_lease_workload
                .read_latency_p95
                .as_micros()
        ...
        println!(
            "- **Throughput**: {:.1} ops/s",
            impl_result.actor_lease_workload.throughput
        );
        println!(
            "- **Peak Memory**: {:.2} MB",
            impl_result.actor_lease_workload.peak_memory_usage as f64 / 1024.0 / 1024.0
        );
        println!(
            "- **Recovery Time**: {:.2}s",
            impl_result
