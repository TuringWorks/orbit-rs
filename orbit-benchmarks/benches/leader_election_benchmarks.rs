...
use orbit_shared::consensus::{RaftConfig, RaftConsensus};
use orbit_shared::election_metrics::{ElectionMetrics, ElectionOutcome};
use orbit_shared::election_state::{ElectionRecord, ElectionStateManager};
    ...
    group.bench_function("election_tracking", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                let node_id = NodeId::new("bench-node".to_string(), "bench".to_string());
                metrics
                    .record_vote_latency(&node_id, Duration::from_millis(fastrand::u64(1..100)))
                    .await;
                metrics.record_message_loss(&node_id, 10, 9).await;
                metrics
                    .record_performance(fastrand::f64() * 100.0, fastrand::u64(1024..1024 * 1024))
                    .await;
        ...
        let metrics = rt.block_on(async {
            ...
            for _ in 0..100 {
                ...
                metrics
                    ...
                    .await;
                metrics
                    .record_message_loss(&node_id, 10, fastrand::u64(8..10))
                    .await;
...
fn bench_concurrent_operations(c: &mut Criterion) {
    ...
    // Benchmark concurrent state operations (simplified to avoid file system races)
    group.bench_function("concurrent_state_operations", |b| {
        b.iter(|| {
            rt.block_on(async {
                // Create separate state managers for each task to avoid file conflicts
            ...
            rt.block_on(async {
                ...
                for i in 0..10 {
                    ...
                    let task = tokio::spawn(async move {
                        ...
                        metrics
                            .record_vote_latency(&node_id, Duration::from_millis(50))
                            .await;
                        metrics.record_message_loss(&node_id, 10, 9).await;
        ...
        b.iter(|| {
            rt.block_on(async {
                ...
                for partition_size in [2, 3, 5] {
                    let affected_nodes = nodes.iter().take(partition_size).cloned().collect();
    ...
    group.bench_function("complete_election_cycle", |b| {
        b.iter(|| {
            rt.block_on(async {
                ...
                for (i, _consensus) in cluster.iter().enumerate() {
                ...
                for (i, metric) in metrics.iter().enumerate() {
                    ...
                    let node_id = NodeId::new(format!("node-{}", i), "bench".to_string());
                    metric
                        .record_vote_latency(&node_id, Duration::from_millis(25))
                        .await;
