...
use orbit_compute::graph_traversal::{GPUGraphTraversal, GraphData, TraversalConfig, NodeProperties};
...
fn generate_graph(node_count: usize, edges_per_node: usize) -> GraphData {
    ...
    for i in 0..node_count {
        ...
        node_properties.insert(node_id, NodeProperties {
            importance: 1.0,
            node_type: None,
        ...
        group.bench_with_input(
            ...
            |b, graph| {
                ...
                b.to_async(&rt).iter(|| async {
                    ...
                    let traversal = GPUGraphTraversal::new(config).await.unwrap();
                    black_box(traversal.detect_communities(graph, min_community_size).await.unwrap())
                });
            },
        );
    ...
    for (node_count, edges_per_node) in graph_sizes {
        ...
        group.bench_with_input(
            ...
            |b, graph| {
                ...
                b.iter(|| {
                    black_box(traversal.prepare_gpu_graph(graph).unwrap())
                });
            },
        );
    }
