use super::{ActorKey, ActorLease, PersistenceMetrics, PersistenceProvider};
        ...
        while start_time.elapsed() < duration {
            ...
            if rand_val < self.query_ratio {
                ...
                if let Some(lease_key) = self.select_random_lease(&active_leases) {
                    let metrics = self
                        .perform_lease_renewal(provider, &lease_key, &mut active_leases)
                        .await?;
            ...
            if operation_start.duration_since(last_throughput_sample) >= Duration::from_secs(1) {
                ...
                results
                    ...
                    .push((operation_start, stats.memory_usage_bytes));
                results
                    .throughput_over_time
                    .push((operation_start, ops_since_last_sample));
    ...
    pub fn calculate_stats(&self) -> WorkloadStats {
        WorkloadStats {
            ...
            average_throughput: self
                ...
                .map(|(_, tps)| *tps)
                .sum::<u64>() as f64
                / self.throughput_over_time.len().max(1) as f64,
        }
    }
