...
use crate::protocols::mcp::sql_generator::SqlGenerator;
use std::sync::Arc;
...
impl McpServer {
    ...
    pub async fn process_natural_language_query(
        ...
        let generated_query = self
            .sql_generator
            .generate_sql(&intent)
            .map_err(|e| crate::protocols::mcp::types::McpError::SqlError(e.to_string()))?;
    ...
    pub async fn execute_natural_language_query(
        ...
        if let Some(ref integration) = self.orbit_integration {
            ...
            Err(crate::protocols::mcp::types::McpError::InternalError(
