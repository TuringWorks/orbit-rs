...
unsafe fn filter_i32_eq_neon(values: &[i32], target: i32) -> Vec<usize> {
    ...
    while i + 4 <= values.len() {
        ...
        for (j, item) in mask.iter().enumerate() {
            if *item != 0 {
                result.push(i + j);
            }
        }
    ...
    for (j, val) in values.iter().enumerate().skip(i) {
        if *val == target {
            result.push(j);
        }
    }
...
unsafe fn sum_i32_neon(values: &[i32], null_bitmap: &NullBitmap) -> Option<i32> {
    ...
    while i + 16 <= values.len() {
        ...
        if all_valid {
            ...
            // Fall back to scalar for this chunk
            for (j, val) in values.iter().enumerate().skip(i).take(16) {
                if null_bitmap.is_valid(j) {
                    // Accumulate in vector
                    let val_vec = vdupq_n_s32(*val);
    ...
    while i + 4 <= values.len() {
        ...
        if all_valid {
            ...
            for (j, val) in values.iter().enumerate().skip(i).take(4) {
                if null_bitmap.is_valid(j) {
                    let val_vec = vdupq_n_s32(*val);
                    sum_vec = vaddq_s32(sum_vec, val_vec);
                    has_value = true;
...
unsafe fn sum_i64_neon(values: &[i64], null_bitmap: &NullBitmap) -> Option<i64> {
    ...
    while i + 2 <= values.len() {
        ...
        if all_valid {
            ...
            for (j, val) in values.iter().enumerate().skip(i).take(2) {
                if null_bitmap.is_valid(j) {
                    let val_vec = vdupq_n_s64(*val);
...
unsafe fn min_i64_neon(values: &[i64], null_bitmap: &NullBitmap) -> Option<i64> {
    ...
    while i + 2 <= values.len() {
        ...
        if all_valid {
            for val in values.iter().skip(i).take(2) {
...
unsafe fn filter_f32_eq_neon(values: &[f32], target: f32) -> Vec<usize> {
    ...
    while i + 4 <= values.len() {
        ...
        for (j, item) in mask_arr.iter().enumerate() {
            if *item != 0 {
                result.push(i + j);
            }
        }
...
unsafe fn min_f32_neon(values: &[f32], null_bitmap: &NullBitmap) -> Option<f32> {
    ...
    while i + 4 <= values.len() {
        if (0..4).all(|j| null_bitmap.is_valid(i + j)) {
            ...
            for (j, v) in values.iter().enumerate().skip(i).take(4) {
                if null_bitmap.is_valid(j) {
                    let v = *v;
                    if v.is_nan() {
                        return Some(f32::NAN);
    ...
    for (j, v) in values.iter().enumerate().skip(i) {
        if null_bitmap.is_valid(j) {
            let v = *v;
            if v.is_nan() {
                return Some(f32::NAN);
...
unsafe fn min_f64_neon(values: &[f64], null_bitmap: &NullBitmap) -> Option<f64> {
    ...
    while i + 2 <= values.len() {
        if (0..2).all(|j| null_bitmap.is_valid(i + j)) {
            ...
            for (j, v) in values.iter().enumerate().skip(i).take(2) {
                if null_bitmap.is_valid(j) {
                    let v = *v;
                    if v.is_nan() {
                        return Some(f64::NAN);
...
unsafe fn filter_i64_ne_neon(values: &[i64], target: i64) -> Vec<usize> {
    ...
    while i + 2 <= values.len() {
        ...
        for (j, item) in mask64.iter().enumerate() {
            if *item != 0 {
                result.push(i + j);
            }
        }
