...
use crate::protocols::postgres_wire::sql::{
    ast::{AccessMode, IsolationLevel, MergeStatement},
...
pub struct MvccTable {
    ...
    /// Table schema
    pub schema: crate::protocols::postgres_wire::sql::executor::TableSchema,
}
...
impl MvccSqlExecutor {
    ...
    pub async fn get_table_schema(
        &self,
        table_name: &str,
    ) -> ProtocolResult<Option<crate::protocols::postgres_wire::sql::executor::TableSchema>> {
        let tables = self.tables.read().await;
    ...
    pub async fn create_table_with_schema(
        &self,
        name: &str,
        schema: Option<crate::protocols::postgres_wire::sql::executor::TableSchema>,
    ) -> ProtocolResult<()> {
        let mut tables = self.tables.write().await;
