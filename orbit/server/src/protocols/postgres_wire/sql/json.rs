...
impl JsonOperations {
    ...
    /// Extract JSON field by index using -> operator
    pub fn json_extract_index(json: &JsonValue, index: i64) -> ProtocolResult<SqlValue> {
        match json.get(index as usize) {
            Some(value) => Ok(SqlValue::Json(value.clone())),
            None => Ok(SqlValue::Null),
        }
    }
    ...
    pub fn json_extract_text(json: &JsonValue, key: &str) -> ProtocolResult<SqlValue> {
        match json.get(key) {
            Some(value) => Ok(SqlValue::Text(json_value_to_text(value))),
            None => Ok(SqlValue::Null),
        }
    }

    /// Extract JSON field as text by index using ->> operator
    pub fn json_extract_text_index(json: &JsonValue, index: i64) -> ProtocolResult<SqlValue> {
        match json.get(index as usize) {
            Some(value) => Ok(SqlValue::Text(json_value_to_text(value))),
            None => Ok(SqlValue::Null),
        }
    ...
    pub fn json_exists_any(json: &JsonValue, keys: &[String]) -> ProtocolResult<SqlValue> {
        let result = match json {
            ...
            JsonValue::Array(arr) => keys.iter().any(|key| {
                arr.iter().any(|v| {
                    if let JsonValue::String(s) = v {
                        s == key
                    } else {
    ...
    pub fn json_exists_all(json: &JsonValue, keys: &[String]) -> ProtocolResult<SqlValue> {
        let result = match json {
            ...
            JsonValue::Array(arr) => keys.iter().all(|key| {
                arr.iter().any(|v| {
                    if let JsonValue::String(s) = v {
                        s == key
                    } else {
    ...
    pub fn json_concat(left: &JsonValue, right: &JsonValue) -> ProtocolResult<SqlValue> {
        let result = match (left, right) {
            ...
            (JsonValue::Array(arr), other) => {
                let mut merged = arr.clone();
                merged.push(other.clone());
                JsonValue::Array(merged)
            }
            (other, JsonValue::Array(arr)) => {
    ...
    pub fn json_delete(json: &JsonValue, key: &str) -> ProtocolResult<SqlValue> {
        let result = match json {
            ...
            JsonValue::Array(arr) => {
                if let Ok(idx) = key.parse::<usize>() {
                    ...
                    // Remove string value from array
                    let new_arr: Vec<JsonValue> = arr
                        .iter()
                        .filter(|v| {
                            if let JsonValue::String(s) = v {
                                s != key
    ...
    /// Delete path from JSONB using jsonb_delete_path function (or #- operator)
    ...
    pub fn json_object_agg(keys: Vec<SqlValue>, values: Vec<SqlValue>) -> ProtocolResult<SqlValue> {
    ...
    pub fn json_build_object(args: Vec<SqlValue>) -> ProtocolResult<SqlValue> {
        ...
        for chunk in args.chunks(2) {
            ...
            map.insert(key, sql_value_to_json(chunk[1].clone())?);
        }
        Ok(SqlValue::Json(JsonValue::Object(map)))
    }

    /// Build JSON array from values (json_build_array)
    pub fn json_build_array(args: Vec<SqlValue>) -> ProtocolResult<SqlValue> {
        let json_values: Vec<JsonValue> = args
            .into_iter()
            .map(sql_value_to_json)
            .collect::<Result<Vec<_>, _>>()?;
        Ok(SqlValue::Json(JsonValue::Array(json_values)))
    ...
    /// Get type of JSON value (json_typeof)
    pub fn json_typeof(json: &JsonValue) -> ProtocolResult<SqlValue> {
    ...
    /// Get array length (json_array_length)
    pub fn json_array_length(json: &JsonValue) -> ProtocolResult<SqlValue> {
        match json {
            JsonValue::Array(arr) => Ok(SqlValue::Integer(arr.len() as i32)),
            _ => Err(ProtocolError::PostgresError(
                "json_array_length requires an array".to_string(),
            )),
        }
    }

    /// Get object keys (json_object_keys)
    pub fn json_object_keys(json: &JsonValue) -> ProtocolResult<Vec<SqlValue>> {
        match json {
            JsonValue::Object(map) => Ok(map.keys().map(|k| SqlValue::Text(k.clone())).collect()),
            _ => Err(ProtocolError::PostgresError(
                "json_object_keys requires an object".to_string(),
            )),
        }
    }

    /// Strip nulls from JSON (json_strip_nulls)
    ...
    match (left, right) {
        ...
        (JsonValue::Array(left_arr), JsonValue::Array(right_arr)) => {
            right_arr.iter().all(|right_val| {
                left_arr
                    .iter()
                    .any(|left_val| contains_json(left_val, right_val))
            })
        }
...
pub fn sql_value_to_json(value: SqlValue) -> ProtocolResult<JsonValue> {
    match value {
...
/// Helper to strip null values from JSON
fn strip_nulls(value: &JsonValue) -> JsonValue {
    match value {
        JsonValue::Object(map) => {
            let new_map: Map<String, JsonValue> = map
                .iter()
...
mod tests {
    ...
    fn test_json_exists() {
        ...
        assert_eq!(
            JsonOperations::json_exists(&json, "a").unwrap(),
            SqlValue::Boolean(true)
        );
        assert_eq!(
            JsonOperations::json_exists(&json, "c").unwrap(),
    ...
    fn test_json_concat() {
        let left = json!({"a": 1});
        let right = json!({"b": 2});
        let result = JsonOperations::json_concat(&left, &right).unwrap();
        match result {
            SqlValue::Jsonb(v) => assert_eq!(v, json!({"a": 1, "b": 2})),
    ...
    fn test_json_delete() {
        let json = json!({"a": 1, "b": 2});
        let result = JsonOperations::json_delete(&json, "a").unwrap();
        match result {
            SqlValue::Jsonb(v) => assert_eq!(v, json!({"b": 2})),
            _ => panic!("Expected JSONB value"),
        }
    }
    ...
    fn test_json_typeof() {
        assert_eq!(
            JsonOperations::json_typeof(&json!(null)).unwrap(),
            SqlValue::Text("null".to_string())
        );
        assert_eq!(
            JsonOperations::json_typeof(&json!(true)).unwrap(),
            SqlValue::Text("boolean".to_string())
        );
        assert_eq!(
            JsonOperations::json_typeof(&json!(42)).unwrap(),
            SqlValue::Text("number".to_string())
        );
        assert_eq!(
            JsonOperations::json_typeof(&json!("hello")).unwrap(),
            SqlValue::Text("string".to_string())
        );
        assert_eq!(
            JsonOperations::json_typeof(&json!([1, 2, 3])).unwrap(),
            SqlValue::Text("array".to_string())
        );
        assert_eq!(
            JsonOperations::json_typeof(&json!({"a": 1})).unwrap(),
