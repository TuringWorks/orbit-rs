...
mod tests {
    ...
    async fn test_window_frames_rows() {
        ...
        engine.execute("CREATE TABLE frame_test (id INT, val INT)").await.unwrap();
        // Insert: 1, 10; 2, 20; 3, 30; 4, 40; 5, 50
        engine.execute("INSERT INTO frame_test VALUES (1, 10), (2, 20), (3, 30), (4, 40), (5, 50)").await.unwrap();
        ...
        match result {
            Ok(UnifiedExecutionResult::Select { rows, .. }) => {
                 ...
                 let get_sum = |r: &Vec<Option<String>>| r[2].as_ref().unwrap().parse::<i64>().unwrap();
    ...
    async fn test_window_frames_range() {
        ...
        engine.execute("CREATE TABLE range_test (id INT, val INT)").await.unwrap();
        engine.execute("INSERT INTO range_test VALUES (1, 10), (1, 20), (2, 30)").await.unwrap();
