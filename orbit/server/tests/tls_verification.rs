...
fn setup_certificates(test_dir: &std::path::Path) -> (PathBuf, PathBuf, PathBuf) {
    ...
    let status = std::process::Command::new("openssl")
        .args(&["req", "-new", "-x509", "-days", "1", "-nodes", "-text", "-out", ca_cert_path.to_str().unwrap(), "-keyout", test_dir.join("ca.key").to_str().unwrap(), "-subj", subject])
        .output()
    ...
    let status = std::process::Command::new("openssl")
        .args(&["req", "-new", "-nodes", "-text", "-out", test_dir.join("server.csr").to_str().unwrap(), "-keyout", server_key_path.to_str().unwrap(), "-subj", subject])
        .output()
        .expect("failed to execute openssl");
    if !status.status.success() { panic!("openssl csr failed"); }

    // Sign Server CSR with CA and include SAN
    let status = std::process::Command::new("openssl")
        .args(&["x509", "-req", "-in", test_dir.join("server.csr").to_str().unwrap(), "-text", "-days", "1", "-CA", ca_cert_path.to_str().unwrap(), "-CAkey", test_dir.join("ca.key").to_str().unwrap(), "-CAcreateserial", "-out", server_cert_path.to_str().unwrap(), "-extfile", ext_path.to_str().unwrap()])
        .output()
        .expect("failed to execute openssl");
    if !status.status.success() { panic!("openssl sign failed"); }
...
async fn test_mongodb_tls_handshake() {
    ...
    let server = MongoDbServer::new(addr.clone())
    ...
    let stream = TcpStream::connect(&addr).await.expect("Failed to connect to Mongo port");
    let mut tls_stream = connector.connect(dns_name, stream).await.expect("TLS handshake failed");
...
async fn test_cypher_tls_handshake() {
    ...
    let server = CypherServer::new_with_storage(&addr, storage)
        .with_tls_config(Some(tls_config));
    ...
    let stream = TcpStream::connect(&addr).await.expect("Failed to connect to Cypher port");
    let mut tls_stream = connector.connect(dns_name, stream).await.expect("TLS handshake failed");
    
    // Bolt Handshake: 0x60 60 B0 17 (GOGOBOLT) + 4 versions
    let handshake = [0x60, 0x60, 0xB0, 0x17, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00];
    tls_stream.write_all(&handshake).await.unwrap();
    // Server should respond with selected version (4 bytes)
    let mut response = [0u8; 4];
    tls_stream.read_exact(&mut response).await.expect("Failed to read Bolt handshake response");
...
async fn test_aql_tls_handshake() {
    ...
    let server = AqlServer::new_with_storage(&addr, storage)
        .with_tls_config(Some(tls_config));
    ...
    let stream = TcpStream::connect(&addr).await.expect("Failed to connect to AQL port");
    let mut tls_stream = connector.connect(dns_name, stream).await.expect("TLS handshake failed");
    
    // Simple HTTP GE request
    let request = b"GET /_api/version HTTP/1.1\r\nHost: localhost\r\n\r\n";
    tls_stream.write_all(request).await.expect("Failed to write HTTP request");
    
    let mut buf = [0u8; 1024];
    let n = tls_stream.read(&mut buf).await.expect("Failed to read HTTP response");
