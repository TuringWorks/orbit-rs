...
impl CqlTestContext {
    ...
    pub async fn new() -> Self {
        ...
        let adapter = CqlAdapter::with_query_engine(config, query_engine.clone()).await.unwrap();
    ...
    pub async fn verify_data(&self, sql: &str) -> Result<orbit_protocols::postgres_wire::sql::executor::ExecutionResult, orbit_protocols::error::ProtocolError> {
        ...
        match result {
            orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { columns, rows, row_count, .. } => {
                Ok(orbit_protocols::postgres_wire::sql::executor::ExecutionResult::Select {
                    columns,
                    rows,
                    row_count,
                })
            ...
            orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Insert { count, .. } => {
                Ok(orbit_protocols::postgres_wire::sql::executor::ExecutionResult::Insert {
                    count,
                })
            }
            orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Update { count, .. } => {
                Ok(orbit_protocols::postgres_wire::sql::executor::ExecutionResult::Update {
                    count,
                })
            }
            orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Delete { count, .. } => {
                Ok(orbit_protocols::postgres_wire::sql::executor::ExecutionResult::Delete {
                    count,
                })
            }
            orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::CreateTable { table_name, .. } => {
                Ok(orbit_protocols::postgres_wire::sql::executor::ExecutionResult::CreateTable {
                    table_name,
                })
            }
            _ => {
                Err(orbit_protocols::error::ProtocolError::PostgresError(
...
pub async fn create_test_users_table(context: &CqlTestContext) {
    context.setup_table("test_users", "(id INTEGER, name TEXT, age INTEGER)").await;
}

/// Helper to create a test table with email
pub async fn create_test_users_with_email_table(context: &CqlTestContext) {
    context.setup_table("test_users", "(id INTEGER, name TEXT, email TEXT)").await;
}
