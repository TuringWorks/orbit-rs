...
impl MySqlSyntaxTestContext {
    ...
    pub async fn try_execute_query(&self, sql: &str) -> Result<orbit_protocols::postgres_wire::sql::UnifiedExecutionResult, orbit_protocols::error::ProtocolError> {
        let mut engine = self.adapter.sql_engine().write().await;
...
async fn test_mysql_integer_types() {
    ...
    let result = ctx.execute_query("SELECT * FROM test_ints WHERE id = 1").await;
    match result {
...
async fn test_mysql_string_types() {
    ...
    let result = ctx.execute_query("SELECT * FROM test_strings WHERE id = 1").await;
    match result {
        orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { rows, .. } => {
            assert_eq!(rows.len(), 1);
        }
...
async fn test_mysql_decimal_types() {
    ...
    let result = ctx.execute_query("SELECT * FROM test_decimals WHERE id = 1").await;
    match result {
        orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { rows, .. } => {
            assert_eq!(rows.len(), 1);
        }
...
async fn test_mysql_date_types() {
    ...
    let result = ctx.execute_query("SELECT * FROM test_dates WHERE id = 1").await;
    match result {
        orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { rows, .. } => {
            assert_eq!(rows.len(), 1);
        }
...
async fn test_mysql_select_limit() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_users", "(id INT PRIMARY KEY, name TEXT)").await;
    
    // Insert multiple rows
    for i in 1..=10 {
        ctx.execute_query(&format!("INSERT INTO test_users (id, name) VALUES ({}, 'user{}')", i, i)).await;
    }
...
async fn test_mysql_select_limit_offset() {
    ...
    let result = ctx.execute_query("SELECT * FROM test_users LIMIT 5 OFFSET 2").await;
    match result {
        orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { rows, .. } => {
...
async fn test_mysql_select_order_by() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_users", "(id INT PRIMARY KEY, name TEXT, age INT)").await;
    
    ctx.execute_query("INSERT INTO test_users (id, name, age) VALUES (1, 'Alice', 30)").await;
    ctx.execute_query("INSERT INTO test_users (id, name, age) VALUES (2, 'Bob', 25)").await;
    ctx.execute_query("INSERT INTO test_users (id, name, age) VALUES (3, 'Charlie', 35)").await;
    
    // Test ORDER BY
    let result = ctx.execute_query("SELECT * FROM test_users ORDER BY age").await;
    match result {
        orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { rows, .. } => {
...
async fn test_mysql_select_order_by_desc() {
    ...
    // Test ORDER BY DESC
    let result = ctx.execute_query("SELECT * FROM test_users ORDER BY age DESC").await;
    match result {
        orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { rows, .. } => {
...
async fn test_mysql_select_distinct() {
    ...
    ctx.execute_query("INSERT INTO test_users (id, name, age) VALUES (1, 'Alice', 30)").await;
    ctx.execute_query("INSERT INTO test_users (id, name, age) VALUES (2, 'Alice', 25)").await;
    
    // Test DISTINCT
    let result = ctx.execute_query("SELECT DISTINCT name FROM test_users").await;
    match result {
        orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { rows, .. } => {
...
async fn test_mysql_select_count() {
    ...
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (1, 'Alice')").await;
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (2, 'Bob')").await;
...
async fn test_mysql_select_aggregate_functions() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_sales", "(id INT PRIMARY KEY, amount INT)").await;
    
    ctx.execute_query("INSERT INTO test_sales (id, amount) VALUES (1, 100)").await;
    ctx.execute_query("INSERT INTO test_sales (id, amount) VALUES (2, 200)").await;
    ctx.execute_query("INSERT INTO test_sales (id, amount) VALUES (3, 300)").await;
...
async fn test_mysql_where_comparison_operators() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_users", "(id INT PRIMARY KEY, age INT)").await;
    
    ctx.execute_query("INSERT INTO test_users (id, age) VALUES (1, 20)").await;
    ctx.execute_query("INSERT INTO test_users (id, age) VALUES (2, 30)").await;
    ctx.execute_query("INSERT INTO test_users (id, age) VALUES (3, 40)").await;
...
async fn test_mysql_where_in_operator() {
    ...
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (1, 'Alice')").await;
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (2, 'Bob')").await;
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (3, 'Charlie')").await;
...
async fn test_mysql_where_is_null() {
    ...
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (1, 'Alice')").await;
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (2, NULL)").await;
...
async fn test_mysql_insert_values() {
    ...
    let result = ctx.execute_query("SELECT * FROM test_users WHERE id = 1").await;
    match result {
        orbit_protocols::postgres_wire::sql::UnifiedExecutionResult::Select { rows, .. } => {
            assert_eq!(rows.len(), 1);
        }
...
async fn test_mysql_insert_multiple_values() {
    ...
    let multi_result = ctx.try_execute_query("INSERT INTO test_users (id, name) VALUES (3, 'Charlie'), (4, 'Dave')").await;
...
async fn test_mysql_insert_select() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_users", "(id INT PRIMARY KEY, name TEXT)").await;
    ctx.setup_table("test_users_copy", "(id INT PRIMARY KEY, name TEXT)").await;
    
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (1, 'Alice')").await;
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (2, 'Bob')").await;
    
    // Test INSERT ... SELECT
    ctx.execute_query("INSERT INTO test_users_copy (id, name) SELECT id, name FROM test_users").await;
...
async fn test_mysql_update_set() {
    ...
    // Test UPDATE SET
    ctx.execute_query("UPDATE test_users SET age = 31 WHERE id = 1").await;
...
async fn test_mysql_update_multiple_columns() {
    ...
    ctx.execute_query("UPDATE test_users SET name = 'Alice Updated', age = 31 WHERE id = 1").await;
...
async fn test_mysql_delete_where() {
    ...
    // Test DELETE WHERE
    ctx.execute_query("DELETE FROM test_users WHERE id = 1").await;
...
async fn test_mysql_inner_join() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_users", "(id INT PRIMARY KEY, name TEXT)").await;
    ctx.setup_table("test_orders", "(id INT PRIMARY KEY, user_id INT, amount INT)").await;
    
    ctx.execute_query("INSERT INTO test_users (id, name) VALUES (1, 'Alice')").await;
    ctx.execute_query("INSERT INTO test_orders (id, user_id, amount) VALUES (1, 1, 100)").await;
...
async fn test_mysql_group_by() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_sales", "(id INT PRIMARY KEY, category TEXT, amount INT)").await;
    
    ctx.execute_query("INSERT INTO test_sales (id, category, amount) VALUES (1, 'A', 100)").await;
    ctx.execute_query("INSERT INTO test_sales (id, category, amount) VALUES (2, 'A', 200)").await;
    ctx.execute_query("INSERT INTO test_sales (id, category, amount) VALUES (3, 'B', 150)").await;
...
async fn test_mysql_subquery_in_where() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_users", "(id INT PRIMARY KEY, name TEXT, age INT)").await;
    ctx.setup_table("test_orders", "(id INT PRIMARY KEY, user_id INT)").await;
    
    ctx.execute_query("INSERT INTO test_users (id, name, age) VALUES (1, 'Alice', 30)").await;
    ctx.execute_query("INSERT INTO test_orders (id, user_id) VALUES (1, 1)").await;
    ...
    let subquery_result = ctx.try_execute_query("SELECT * FROM test_users WHERE id IN (SELECT user_id FROM test_orders)").await;
...
async fn test_mysql_date_functions() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_events", "(id INT PRIMARY KEY, event_date TEXT)").await;
    
    ctx.execute_query("INSERT INTO test_events (id, event_date) VALUES (1, '2024-01-01')").await;
...
async fn test_mysql_numeric_functions() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_numbers", "(id INT PRIMARY KEY, value INT)").await;
    
    ctx.execute_query("INSERT INTO test_numbers (id, value) VALUES (1, 25)").await;
...
async fn test_mysql_create_table_with_constraints() {
    ...
    ctx.setup_table("test_pk", "(id INT PRIMARY KEY, name TEXT)").await;
    
    // Test CREATE TABLE with NOT NULL
    ctx.setup_table("test_nn", "(id INT PRIMARY KEY, name TEXT NOT NULL)").await;
...
async fn test_mysql_alter_table() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_alter", "(id INT PRIMARY KEY, name TEXT)").await;
    
    // Test ALTER TABLE ADD COLUMN
    ctx.execute_query("ALTER TABLE test_alter ADD COLUMN age INT").await;
    
    // Verify column was added
    ctx.execute_query("INSERT INTO test_alter (id, name, age) VALUES (1, 'Alice', 30)").await;
...
async fn test_mysql_drop_table() {
    let ctx = MySqlSyntaxTestContext::new().await;
    ctx.setup_table("test_drop", "(id INT PRIMARY KEY, name TEXT)").await;
...
async fn test_mysql_prepared_statement_syntax() {
    ...
    for query in queries_without_params {
        let param_count = MySqlAdapter::count_parameters(query);
        assert_eq!(param_count, 0, "Query should not have parameters: {}", query);
    }
}
