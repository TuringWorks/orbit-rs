...
async fn test_cql_select_with_where_integration() {
    ...
    ctx.insert_data_sql("INSERT INTO test_users (id, name) VALUES (1, 'Alice')")
        .await;

    // Verify data was inserted
    match ctx
        .verify_data("SELECT * FROM test_users WHERE id = 1")
        .await
    {
    ...
    match result {
        Ok(frame) => {
            if frame.opcode == orbit_protocols::cql::protocol::CqlOpcode::Error {
                ...
                panic!(
                    "CQL query returned Error opcode: code={}, message={}",
...
async fn test_cql_update_execution_integration() {
    ...
    let verify_result = ctx
        .verify_data("SELECT email FROM test_users WHERE id = 1")
        .await;
    match verify_result {
...
async fn test_cql_delete_execution_integration() {
    ...
    ctx.insert_data_sql("INSERT INTO test_users (id, name) VALUES (1, 'Alice')")
        .await;
    ctx.insert_data_sql("INSERT INTO test_users (id, name) VALUES (2, 'Bob')")
        .await;
    ...
    match verify_result {
        Ok(ExecutionResult::Select { columns, rows, .. }) => {
            ...
            assert_eq!(
                rows.len(),
                1,
                "Only Bob (id=2) should remain after deleting id=1"
            );
                ...
                if let Some(row) = rows.first() {
                    if row.len() > id_pos.max(name_pos) {
                        ...
                        assert_eq!(
                            id,
                            Some("2"),
                            "Remaining row should have id=2, got: {:?}",
                            id
                        );
                        assert_eq!(
                            name,
                            Some("Bob"),
                            "Remaining row should be Bob, got: {:?}",
                            name
                        );
                    } else {
                        panic!("Row too short: {:?}, columns: {:?}", row, columns);
