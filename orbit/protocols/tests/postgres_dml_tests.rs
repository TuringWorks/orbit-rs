...
async fn test_insert_all_types() {
    ...
    let sql =
        "INSERT INTO all_types VALUES (42, 9223372036854775807, 'hello world', true, 3.14159)";
    let result = executor.execute(sql).await.unwrap();
...
async fn test_insert_text() {
    ...
    // Create table
    executor
        .execute("CREATE TABLE messages (text TEXT)")
        .await
        .unwrap();
...
async fn test_insert_booleans() {
    ...
    // Create table
    executor
        .execute("CREATE TABLE flags (flag BOOLEAN)")
        .await
        .unwrap();
...
async fn test_update_all_rows() {
    ...
    // Create and populate table
    executor
        .execute("CREATE TABLE users (id INTEGER, active BOOLEAN)")
        .await
        .unwrap();
    executor
        .execute("INSERT INTO users VALUES (1, true), (2, true), (3, false)")
        .await
        .unwrap();
...
async fn test_update_to_null() {
    ...
    executor
        ...
        .unwrap();
    executor
        .execute("INSERT INTO users VALUES (1, 'alice@example.com')")
        .await
        .unwrap();
...
async fn test_update_with_integer_comparison() {
    ...
    executor
        ...
        .unwrap();
    executor
        .execute("INSERT INTO products VALUES (1, 100), (2, 200), (3, 50)")
        .await
        .unwrap();
...
async fn test_delete_multiple_rows() {
    ...
    executor
        ...
        .unwrap();
    executor
        .execute("INSERT INTO users VALUES (1, false), (2, false), (3, true)")
        .await
        .unwrap();
...
async fn test_delete_all_rows() {
    ...
    // Create and populate table
    executor
        .execute("CREATE TABLE temp_data (value INTEGER)")
        .await
        .unwrap();
    executor
        .execute("INSERT INTO temp_data VALUES (1), (2), (3)")
        .await
        .unwrap();
...
async fn test_delete_with_comparison() {
    ...
    // Create and populate table
    executor
        .execute("CREATE TABLE scores (player TEXT, score INTEGER)")
        .await
        .unwrap();
    executor
        .execute("INSERT INTO scores VALUES ('Alice', 100), ('Bob', 50), ('Charlie', 150)")
        .await
        .unwrap();
...
async fn test_insert_update_delete_flow() {
    ...
    // Create table
    executor
        .execute("CREATE TABLE workflow (id INTEGER, status TEXT)")
        .await
        .unwrap();

    // INSERT
    let result = executor
        .execute("INSERT INTO workflow VALUES (1, 'pending')")
        .await
        .unwrap();
    assert!(matches!(result, ExecutionResult::Insert { count: 1 }));
    ...
    // UPDATE
    let result = executor
        .execute("UPDATE workflow SET status = 'completed' WHERE id = 1")
        .await
        .unwrap();
    ...
    // DELETE
    let result = executor
        .execute("DELETE FROM workflow WHERE id = 1")
        .await
        .unwrap();
...
async fn test_bulk_operations() {
    ...
    // Create table
    executor
        .execute("CREATE TABLE bulk_test (id INTEGER, value TEXT)")
        .await
        .unwrap();

    // Bulk insert
    executor
        .execute("INSERT INTO bulk_test VALUES (1, 'a'), (2, 'b'), (3, 'c'), (4, 'd'), (5, 'e')")
        .await
        .unwrap();

    // Bulk update
    let result = executor
        .execute("UPDATE bulk_test SET value = 'updated'")
        .await
        .unwrap();
    match result {
        ExecutionResult::Update { count } => {
...
async fn test_sequential_inserts() {
    ...
    // Create table
    executor
        .execute("CREATE TABLE sequence (id INTEGER)")
        .await
        .unwrap();
