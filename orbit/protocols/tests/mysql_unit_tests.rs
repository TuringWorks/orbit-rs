...
fn test_auth_plugin_conversion() {
    ...
    assert_eq!(
        AuthPlugin::CachingSha2Password.as_str(),
...
fn test_error_code_mapping() {
    ...
    // Table not found
    assert_eq!(
        map_error_to_mysql_code(&ProtocolError::PostgresError(
    ...
    assert_eq!(
        map_error_to_mysql_code(&ProtocolError::AuthenticationError(
...
fn test_count_parameters() {
    ...
    // Single parameter
    assert_eq!(
        MySqlAdapter::count_parameters("SELECT * FROM users WHERE id = ?"),
        1
    );
...
fn test_map_error_to_mysql_code_enhanced() {
    ...
    assert_eq!(
        map_error_to_mysql_code(&ProtocolError::PostgresError(
            "database access denied".to_string()
        )),
    ...
    assert_eq!(
        map_error_to_mysql_code(&ProtocolError::PostgresError(
            "column access denied".to_string()
        )),
