...
use bytes::Buf;
use orbit_protocols::cql::{CqlAdapter, CqlConfig, CqlParser, CqlStatement, ComparisonOperator};
...
fn test_parse_where_comparison_operators() {
    ...
    if let CqlStatement::Select { where_clause, .. } = stmt {
        let conditions = where_clause.unwrap();
        assert_eq!(conditions[0].operator, ComparisonOperator::GreaterThanOrEqual);
    }
    ...
    // Test !=
    let stmt = parser.parse("SELECT * FROM users WHERE status != 'deleted'").unwrap();
    if let CqlStatement::Select { where_clause, .. } = stmt {
        let conditions = where_clause.unwrap();
...
fn test_parse_where_multiple_conditions() {
    let parser = CqlParser::new();
    let stmt = parser.parse("SELECT * FROM users WHERE age > 25 AND status = 'active'").unwrap();
...
fn test_parse_where_in_operator() {
    let parser = CqlParser::new();
    let stmt = parser.parse("SELECT * FROM users WHERE id IN (1, 2, 3)").unwrap();
...
fn test_parse_insert_with_values() {
    let parser = CqlParser::new();
    let stmt = parser.parse("INSERT INTO users (id, name, age) VALUES (1, 'Alice', 30)").unwrap();
    
    match stmt {
        CqlStatement::Insert { columns, values, .. } => {
...
fn test_parse_insert_with_ttl() {
    let parser = CqlParser::new();
    let stmt = parser.parse("INSERT INTO users (id, name) VALUES (1, 'Alice') USING TTL 3600").unwrap();
...
fn test_parse_update_with_set() {
    let parser = CqlParser::new();
    let stmt = parser.parse("UPDATE users SET email = 'new@example.com', age = 31 WHERE id = 1").unwrap();
    
    match stmt {
        CqlStatement::Update { assignments, where_clause, .. } => {
...
fn test_parse_delete_specific_columns() {
    let parser = CqlParser::new();
    let stmt = parser.parse("DELETE email, phone FROM users WHERE id = 1").unwrap();
...
async fn test_cql_select_execution() {
    ...
    let insert_stmt1 = parser.parse("INSERT INTO test_users (id, name, age) VALUES (1, 'Alice', 30)").unwrap();
...
async fn test_cql_select_with_where() {
    ...
    adapter.execute_sql("CREATE TABLE test_users (id INTEGER, name TEXT)").await.unwrap();
    adapter.execute_sql("INSERT INTO test_users (id, name) VALUES (1, 'Alice')").await.unwrap();
...
async fn test_cql_insert_execution() {
    ...
    adapter.execute_sql("CREATE TABLE test_users (id INTEGER, name TEXT, age INTEGER)").await.unwrap();
    ...
    let select_result = adapter.execute_sql("SELECT * FROM test_users WHERE id = 1").await.unwrap();
    match select_result {
...
async fn test_cql_update_execution() {
    ...
    // Setup: Create table and insert data using adapter's SQL engine (shared storage)
    adapter.execute_sql("CREATE TABLE test_users (id INTEGER, name TEXT, email TEXT)").await.unwrap();
    adapter.execute_sql("INSERT INTO test_users (id, name, email) VALUES (1, 'Alice', 'old@example.com')").await.unwrap();
    ...
    let select_result = adapter.execute_sql("SELECT email FROM test_users WHERE id = 1").await.unwrap();
    match select_result {
...
async fn test_cql_delete_execution() {
    ...
    adapter.execute_sql("CREATE TABLE test_users (id INTEGER, name TEXT)").await.unwrap();
    adapter.execute_sql("INSERT INTO test_users (id, name) VALUES (1, 'Alice')").await.unwrap();
    adapter.execute_sql("INSERT INTO test_users (id, name) VALUES (2, 'Bob')").await.unwrap();
    ...
    let select_result = adapter.execute_sql("SELECT * FROM test_users").await.unwrap();
    match select_result {
        orbit_protocols::postgres_wire::sql::execution_strategy::UnifiedExecutionResult::Select { row_count, .. } => {
...
fn test_frame_encoding_decoding() {
    ...
    let original = CqlFrame::new(
        CqlOpcode::Query,
        Bytes::from("SELECT * FROM users")
    );
