...
fn format_as_table(columns: &[String], rows: &[HashMap<String, SqlValue>]) -> String {
    ...
    let header_cells: Vec<Cell> = columns
        .iter()
        .map(|col| Cell::new(col).fg(Color::Cyan).set_alignment(comfy_table::CellAlignment::Center))
        .collect();
    ...
    let summary = if rows.is_empty() {
        ...
        format!("{} row{}", rows.len(), if rows.len() == 1 { "" } else { "s" })
    };
...
fn format_cell_value(value: &SqlValue) -> Cell {
    match value {
        SqlValue::Null => Cell::new("NULL").fg(Color::Grey).set_alignment(comfy_table::CellAlignment::Center),
        SqlValue::Boolean(b) => {
            let text = if *b { "true" } else { "false" };
            Cell::new(text).fg(Color::Yellow).set_alignment(comfy_table::CellAlignment::Center)
        }
        SqlValue::SmallInt(n) => {
            Cell::new(n.to_string()).fg(Color::Green).set_alignment(comfy_table::CellAlignment::Right)
        }
        SqlValue::Integer(n) => {
            Cell::new(n.to_string()).fg(Color::Green).set_alignment(comfy_table::CellAlignment::Right)
        }
        ...
        SqlValue::Real(f) => {
            Cell::new(format!("{:.2}", f)).fg(Color::Green).set_alignment(comfy_table::CellAlignment::Right)
        }
        SqlValue::DoublePrecision(f) => {
            Cell::new(format!("{:.2}", f)).fg(Color::Green).set_alignment(comfy_table::CellAlignment::Right)
        }
        SqlValue::Decimal(d) => {
            Cell::new(d.to_string()).fg(Color::Green).set_alignment(comfy_table::CellAlignment::Right)
        }
        SqlValue::Text(s) | SqlValue::Varchar(s) | SqlValue::Char(s) => {
            Cell::new(s).fg(Color::White).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Bytea(bytes) => {
            ...
            Cell::new(format!("\\x{}", display)).fg(Color::Magenta).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Timestamp(ts) => {
            Cell::new(ts.to_string()).fg(Color::Blue).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::TimestampWithTimezone(ts) => {
            Cell::new(ts.to_string()).fg(Color::Blue).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Date(d) => {
            Cell::new(d.to_string()).fg(Color::Blue).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Time(t) => {
            Cell::new(t.to_string()).fg(Color::Blue).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::TimeWithTimezone(t) => {
            Cell::new(t.to_string()).fg(Color::Blue).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Interval(i) => {
            let interval_str = format!("{} months {} days {}μs", i.months, i.days, i.microseconds);
            Cell::new(interval_str).fg(Color::Blue).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Uuid(u) => {
            Cell::new(u.to_string()).fg(Color::Cyan).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Json(j) | SqlValue::Jsonb(j) => {
            ...
            Cell::new(display).fg(Color::Yellow).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Array(items) => {
            let display = format!("[{} items]", items.len());
            Cell::new(display).fg(Color::Magenta).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Point(x, y) => {
            Cell::new(format!("({}, {})", x, y)).fg(Color::Cyan).set_alignment(comfy_table::CellAlignment::Left)
        }
        SqlValue::Inet(s) => {
            Cell::new(s.to_string()).fg(Color::Blue).set_alignment(comfy_table::CellAlignment::Left)
        }
        // Handle remaining types with a generic approach
        _ => {
            Cell::new(value.to_postgres_string()).fg(Color::White).set_alignment(comfy_table::CellAlignment::Left)
        }
    }
}
...
fn sql_value_to_json(value: &SqlValue) -> serde_json::Value {
    match value {
        ...
        SqlValue::Text(s) | SqlValue::Varchar(s) | SqlValue::Char(s) => serde_json::Value::String(s.clone()),
...
pub fn format_timing(elapsed_ms: u64) -> String {
    if elapsed_ms < 1000 {
        ...
        format!("Time: {:.2} s", elapsed_ms as f64 / 1000.0).dimmed().to_string()
    }
}
