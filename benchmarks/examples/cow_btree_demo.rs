...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "   ✅ Renewed lease in {:.1}μs",
        renew_metrics.latency.as_micros()
    );
    println!(
        "   📈 Version updated: {} -> {}",
        lease.version, renewed_lease.version
    );
    println!(
        "   📊 Renewal count: {}",
        renewed_lease.metadata.renewal_count
    );
    ...
    println!(
        "   💾 Data read during recovery: {} bytes",
