...
impl CowBTreePersistence {
    ...
    async fn replay_wal(&mut self, wal_path: &std::path::Path) -> Result<(), PersistenceError> {
        ...
        println!(
            "WAL replay completed. Replayed {} operations",
            replayed_count
        );
        Ok(())
    }
    ...
    async fn apply_insert_for_recovery(
        &mut self,
        key: ActorKey,
        lease: ActorLease,
    ...
    fn cow_insert_recursive(
        ...
        if new_node.is_leaf {
            ...
            let (new_child, child_memory_delta) =
                self.cow_insert_recursive(child, key, lease, new_version)?;
