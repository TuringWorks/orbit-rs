...
#[derive(Debug, Clone, Serialize, Deserialize)]
enum WALOperation {
    InsertLease { key: String, lease: AddressableLease },
    UpdateLease { key: String, lease: AddressableLease },
    DeleteLease { key: String },
    InsertNode { key: String, node: NodeInfo },
    UpdateNode { key: String, node: NodeInfo },
...
impl WriteAheadLog {
    ...
    async fn append_entry(&mut self, operation: WALOperation) -> OrbitResult<()> {
        ...
        self.sequence += 1;
        tracing::debug!("[WAL] Appending entry {} with {} bytes", entry.sequence, serialized.len());
...
impl PersistenceProvider for CowBTreeAddressableProvider {
    async fn initialize(&self) -> OrbitResult<()> {
        ...
        *self.wal.lock().await = wal;
        ...
        tracing::info!("COW B+ Tree addressable provider initialized at {}", data_dir.display());
        Ok(())
    }
...
impl ClusterNodeProvider for CowBTreeClusterProvider {
    ...
    async fn list_active_nodes(&self) -> OrbitResult<Vec<NodeInfo>> {
        let root = self.root.read().unwrap();
        let nodes: Vec<NodeInfo> = root.node_values
            .iter()
            .filter_map(|v| v.clone())
