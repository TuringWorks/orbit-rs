...
impl PersistenceProvider for RocksDbAddressableProvider {
    async fn initialize(&self) -> OrbitResult<()> {
        tracing::info!("RocksDB addressable provider initialized at {}", self.config.data_dir);
        Ok(())
    }
...
impl AddressableDirectoryProvider for RocksDbAddressableProvider {
    ...
    async fn remove_lease(&self, reference: &AddressableReference) -> OrbitResult<bool> {
        ...
        let exists = self.db.get(key.as_bytes()).map(|opt| opt.is_some()).unwrap_or(false);
        ...
        self.update_metrics("delete", start.elapsed(), success).await;
        Ok(exists)
    }
    ...
    async fn store_leases_bulk(&self, leases: &[AddressableLease]) -> OrbitResult<()> {
        ...
        if let Err(e) = result {
            self.update_metrics("write", start.elapsed(), false).await;
            return Err(OrbitError::internal(format!("RocksDB batch write error: {}", e)));
        }
    ...
    async fn remove_leases_bulk(&self, references: &[AddressableReference]) -> OrbitResult<u64> {
        ...
        for reference in references {
            ...
            // Check if key exists
            if self.db.get(key.as_bytes()).map(|opt| opt.is_some()).unwrap_or(false) {
        ...
        if let Err(e) = result {
            self.update_metrics("delete", start.elapsed(), false).await;
            return Err(OrbitError::internal(format!("RocksDB batch delete error: {}", e)));
        }
...
impl RocksDbClusterProvider {
    pub fn new(config: RocksDbConfig) -> OrbitResult<Self> {
        ...
        let db = TransactionDB::open(&opts, &txn_opts, &cluster_data_dir)
            .map_err(|e| OrbitError::internal(format!("Failed to open RocksDB for cluster: {}", e)))?;
