    ...
    C: Send + Sync,
{
    ...
    factory: Arc<dyn Fn(String) -> futures::future::BoxFuture<'static, OrbitResult<C>> + Send + Sync>,
    ...
    C: Send + Sync + 'static,
{
    ...
    where
        F: Fn(String) -> futures::future::BoxFuture<'static, OrbitResult<C>> + Send + Sync + 'static,
    {
    ...
    async fn return_connection(&self, connection: C, node_id: String, is_healthy: bool) {
        ...
        if is_healthy && connections.len() < self.config.max_connections {
            ...
            debug!("Dropping connection (healthy: {}, pool_size: {})", is_healthy, connections.len());
        }
    }
    ...
    async fn adjust_pool_size(&self) {
        ...
        let target_active = (self.config.max_connections as f64 * self.config.target_utilization) as usize;
