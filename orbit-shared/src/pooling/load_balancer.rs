...
impl ConnectionLoadBalancer {
    ...
    pub async fn select_node(&self) -> OrbitResult<Option<String>> {
        ...
        let available_nodes: Vec<&NodeHealth> = nodes
            .values()
        ...
        let selected_node = match self.strategy {
            ...
            LoadBalancingStrategy::WeightedRoundRobin => {
                self.select_weighted(&available_nodes)
            }
    ...
    pub async fn select_node_with_affinity(&self, affinity_key: &str) -> OrbitResult<Option<String>> {
        let nodes = self.nodes.read().await;
