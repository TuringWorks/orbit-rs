...
impl ElectionState {
    ...
    pub fn vote_for(&mut self, candidate: NodeId, term: u64) -> Result<(), String> {
        if term != self.current_term {
            return Err(format!("Term mismatch: expected {}, got {}", self.current_term, term));
        }
        
        if let Some(existing_vote) = &self.voted_for {
            if existing_vote != &candidate {
                return Err(format!("Already voted for {} in term {}", existing_vote, term));
            }
        }
    ...
    pub fn get_election_stats(&self) -> ElectionStats {
        ...
        let successful_elections = self.election_history.iter().filter(|r| r.successful).count();
        
        let recent_elections = self.election_history
            .iter()
            .rev()
            .take(10)
            .cloned()
        ...
        let avg_vote_percentage = if !self.election_history.is_empty() {
            let total_percentage: f64 = self.election_history
                .iter()
...
impl ElectionStateManager {
    ...
    pub async fn load(&self) -> OrbitResult<()> {
        ...
        info!("Loaded election state from disk: term {}", self.get_current_term().await);
        Ok(())
    }
    ...
    pub async fn save(&self) -> OrbitResult<()> {
        ...
        let content = serde_json::to_string_pretty(&*state)
            .map_err(|e| OrbitError::internal(format!("Failed to serialize election state: {}", e)))?;
        
        // Create parent directory if it doesn't exist
        if let Some(parent) = self.state_path.parent() {
            fs::create_dir_all(parent).await
                .map_err(|e| OrbitError::internal(format!("Failed to create state directory: {}", e)))?;
        }
        ...
        fs::rename(&temp_path, &self.state_path).await
            .map_err(|e| OrbitError::internal(format!("Failed to update election state file: {}", e)))?;
    ...
    pub async fn vote_for(&self, candidate: NodeId, term: u64) -> OrbitResult<()> {
        {
            let mut state = self.state.write().await;
            state.vote_for(candidate.clone(), term)
    ...
    pub async fn record_election(&self, record: ElectionRecord) -> OrbitResult<()> {
        ...
        self.save().await?;
        info!("Recorded election: term {}, candidate {}, success: {}", 
