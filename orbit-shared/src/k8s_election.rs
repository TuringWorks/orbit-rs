...
use crate::election_state::{ElectionStateManager, ElectionRecord};
...
pub enum NodeDiscoveryMethod {
    ...
    Etcd { endpoints: Vec<String>, key_prefix: String },
}
...
impl UniversalElectionManager {
    ...
    pub async fn new(
        ...
        let state_manager = match state_path {
            ...
            None => {
                let default_path = std::env::temp_dir().join(format!("orbit-election-{}.json", node_id.key));
        ...
        let raft_manager = if matches!(deployment_mode, DeploymentMode::Standalone { .. }) || k8s_config.enable_raft_fallback {
    ...
    async fn setup_raft_manager(node_id: &NodeId, deployment_mode: &DeploymentMode) -> OrbitResult<Arc<EnhancedClusterManager>> {
    ...
    async fn discover_cluster_nodes(deployment_mode: &DeploymentMode) -> OrbitResult<Vec<NodeId>> {
        match deployment_mode {
            ...
            DeploymentMode::Standalone { node_discovery, .. } => {
                match node_discovery {
                    NodeDiscoveryMethod::Static(nodes) => {
                        Ok(nodes.iter().enumerate().map(|(i, _addr)| {
                            NodeId::new(format!("node-{}", i), "cluster".to_string())
                        }).collect())
                    },
                    NodeDiscoveryMethod::Dns(dns_name) => {
                        Self::discover_dns_nodes(dns_name).await
                    },
                    _ => {
            ...
            DeploymentMode::DockerCompose { compose_project, service_name } => {
    ...
    async fn discover_dns_nodes(dns_name: &str) -> OrbitResult<Vec<NodeId>> {
        ...
        match lookup_host(dns_name).await {
            Ok(addrs) => {
                let nodes: Vec<NodeId> = addrs.enumerate().map(|(i, addr)| {
                    NodeId::new(format!("dns-node-{}-{}", i, addr.ip()), "cluster".to_string())
                }).collect();
    ...
    async fn discover_compose_nodes(project: &str, service: &str) -> OrbitResult<Vec<NodeId>> {
        ...
        let nodes: Vec<NodeId> = (1..=replica_count)
            .map(|i| NodeId::new(format!("{}_{}_{}",project, service, i), "compose".to_string()))
            .collect();
    ...
    pub async fn start(&self) -> OrbitResult<()> {
        info!("Starting universal election manager in mode: {:?}", self.deployment_mode);
    ...
    async fn start_k8s_election(&self) -> OrbitResult<()> {
        ...
        tokio::spawn(async move {
            let mut retry_interval = tokio::time::interval(
                Duration::from_secs(election_manager.k8s_config.retry_period_secs)
            );
    ...
    async fn run_k8s_election_cycle(&self) -> OrbitResult<()> {
        let lease_client = self.lease_client.as_ref()
        ...
        match current_lease {
            Some(lease) => {
                if lease.holder_identity == identity {
                    ...
                    self.renew_leadership(lease_client.as_ref(), lease, now).await?;
                } else {
                    // Someone else is leader, check if lease is expired
                    if self.is_lease_expired(&lease, now) {
                        self.attempt_leadership_acquisition(lease_client.as_ref(), identity, now).await?;
                    } else {
    ...
    async fn renew_leadership(&self, client: &dyn K8sLeaseClient, mut lease: K8sLease, now: SystemTime) -> OrbitResult<()> {
        lease.renew_time = now;
        
        if let Err(e) = client.update_lease(&self.k8s_config.namespace, &self.k8s_config.lease_name, &lease).await {
    ...
    async fn attempt_leadership_acquisition(&self, client: &dyn K8sLeaseClient, identity: String, now: SystemTime) -> OrbitResult<()> {
        ...
        match client.create_lease(&self.k8s_config.namespace, &self.k8s_config.lease_name, &new_lease).await {
            Ok(()) => {
    ...
    fn is_lease_expired(&self, lease: &K8sLease, now: SystemTime) -> bool {
        ...
        let elapsed = now.duration_since(lease.renew_time).unwrap_or(Duration::ZERO);
    ...
    async fn update_follower_state(&self, leader_identity: &str) {
        ...
        *current_leader = if leader_identity.is_empty() {
        ...
        debug!("Updated state: This node is a follower, leader: {:?}", current_leader);
    }
    
    fn get_node_identity(&self) -> String {
        self.k8s_config.pod_name.clone()
    ...
    async fn health_check(&self) -> OrbitResult<()> {
        ...
        if matches!(self.deployment_mode, DeploymentMode::Kubernetes { .. }) {
            if let Some(client) = &self.lease_client {
                let _lease = client.get_lease(&self.k8s_config.namespace, &self.k8s_config.lease_name).await;
    ...
    pub async fn wait_for_leadership(&self, timeout_duration: Duration) -> OrbitResult<()> {
        ...
        Err(OrbitError::timeout("Leadership not acquired within timeout"))
    }
    ...
    pub async fn release_leadership(&self) -> OrbitResult<()> {
        ...
        if let Some(client) = &self.lease_client {
            if let Err(e) = client.delete_lease(&self.k8s_config.namespace, &self.k8s_config.lease_name).await {
...
impl K8sLeaseClient for KubernetesLeaseClient {
    ...
    async fn create_lease(&self, namespace: &str, name: &str, lease: &K8sLease) -> OrbitResult<()> {
        info!("Creating lease {}/{} for {}", namespace, name, lease.holder_identity);
    ...
    async fn update_lease(&self, namespace: &str, name: &str, lease: &K8sLease) -> OrbitResult<()> {
        debug!("Updating lease {}/{} for {}", namespace, name, lease.holder_identity);
...
mod tests {
    ...
    async fn test_deployment_mode_detection() {
        ...
        match mode {
            DeploymentMode::Kubernetes { namespace, pod_name, .. } => {
