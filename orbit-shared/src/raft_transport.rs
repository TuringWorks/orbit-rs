use crate::consensus::{RaftTransport, VoteRequest, VoteResponse, AppendEntriesRequest, AppendEntriesResponse};
...
use raft_service::{
    ...
    VoteRequestProto, VoteResponseProto, 
    AppendEntriesRequestProto, AppendEntriesResponseProto,
};
...
impl GrpcRaftTransport {
    ...
    async fn get_client(&self, target_node: &NodeId) -> OrbitResult<RaftConsensusClient<tonic::transport::Channel>> {
        ...
        let address = {
            ...
            addresses.get(target_node).cloned()
                .ok_or_else(|| OrbitError::cluster(format!("Address not found for node: {}", target_node)))?
        };
...
impl RaftTransport for GrpcRaftTransport {
    async fn send_vote_request(&self, target: &NodeId, request: VoteRequest) -> OrbitResult<VoteResponse> {
        ...
        let result = tokio::time::timeout(
            ...
            client.request_vote(grpc_request)
        ).await;
        ...
        match result {
            ...
            Ok(Err(status)) => {
                ...
                Err(OrbitError::network(&format!("Vote request failed: {}", status)))
            },
            Err(_) => {
                warn!("Vote request to {} timed out", target);
                self.remove_client(target).await;
                Err(OrbitError::timeout(&format!("Vote request timeout to {}", target)))
            }
        }
    }
    ...
    async fn send_append_entries(&self, target: &NodeId, request: AppendEntriesRequest) -> OrbitResult<AppendEntriesResponse> {
        ...
        let proto_request = AppendEntriesRequestProto {
            ...
            entries: request.entries.into_iter().map(|entry| {
                raft_service::LogEntryProto {
                    term: entry.term,
                    index: entry.index,
        ...
        let result = tokio::time::timeout(
            self.request_timeout,
            client.append_entries(grpc_request)
        ).await;
        ...
        match result {
            ...
            Ok(Err(status)) => {
                ...
                self.remove_client(target).await;
                Err(OrbitError::network(&format!("Append entries failed: {}", status)))
            },
            Err(_) => {
                warn!("Append entries to {} timed out", target);
                self.remove_client(target).await;
                Err(OrbitError::timeout(&format!("Append entries timeout to {}", target)))
            }
        }
    }
...
impl RaftConsensusService for GrpcRaftHandler {
    ...
    async fn append_entries(
        ...
        let entries: Vec<crate::consensus::LogEntry> = proto_request.entries
            .into_iter()
            .map(|entry| {
