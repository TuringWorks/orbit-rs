...
mod tests {
    ...
    fn test_all_error_constructors() {
        ...
        assert_eq!(cluster_error.to_string(), "Cluster error: Cluster unreachable");
    ...
    fn test_addressable_not_found_error() {
        ...
        assert_eq!(error.to_string(), "Addressable not found: TestActor:key-123");
    }
    ...
    fn test_lease_expired_error() {
        ...
        assert_eq!(error.to_string(), "Lease expired: Lease for TestActor expired at 2023-01-01T00:00:00Z");
    }
    ...
    fn test_error_matching() {
        let errors = vec![
            OrbitError::NodeNotFound { node_id: "test".to_string() },
            OrbitError::AddressableNotFound { reference: "test".to_string() },
            OrbitError::LeaseExpired { details: "test".to_string() },
            ...
            OrbitError::Timeout { operation: "test".to_string() },
        ...
        for error in errors {
            ...
            match error {
                ...
                OrbitError::AddressableNotFound { .. } => assert!(error_string.contains("Addressable not found")),
                OrbitError::LeaseExpired { .. } => assert!(error_string.contains("Lease expired")),
                OrbitError::InvocationFailed { .. } => assert!(error_string.contains("Invocation failed")),
                OrbitError::SerializationError(_) => assert!(error_string.contains("Serialization error")),
                ...
                OrbitError::ConfigurationError(_) => assert!(error_string.contains("Configuration error")),
