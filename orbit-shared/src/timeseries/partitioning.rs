...
impl PartitionManager {
    ...
    pub fn get_partition_for_point(&self, data_point: &DataPoint) -> Result<String> {
        match &self.strategy {
            PartitionStrategy::TimeInterval(interval) => {
                ...
                Ok(format!("time_{}_{}", partition_start, partition_start + interval_nanos))
            },
    ...
    pub fn create_partition(&mut self, partition_info: PartitionInfo) -> Result<()> {
        self.partitions.insert(partition_info.partition_id.clone(), partition_info);
        Ok(())
    }
    ...
    pub async fn split_partition(&mut self, _partition_id: &str, _split_points: Vec<Timestamp>) -> Result<Vec<PartitionInfo>> {
    ...
    pub fn update_partition_stats(&mut self, partition_id: &str, data_points: u64, size_bytes: u64) {
    ...
    pub async fn cleanup_old_partitions(&mut self, retention_duration: Duration) -> Result<Vec<String>> {
        ...
        let partitions_to_remove: Vec<_> = self.partitions
            .iter()
...
mod tests {
    ...
    fn test_partition_info_creation() {
        let mut manager = PartitionManager::new(PartitionStrategy::TimeInterval(Duration::from_secs(3600)));
        
        let partition_info = PartitionInfo {
            partition_id: "test_partition".to_string(),
            start_time: 0,
            end_time: 3600_000_000_000,
            series_count: 10,
            data_points: 1000,
            size_bytes: 50000,
            created_at: Utc::now(),
