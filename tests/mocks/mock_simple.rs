...
async fn test_simple_mock_framework() -> anyhow::Result<()> {
    ...
    setup
        .register_actor_types(&["SimpleTestActor"])
        .await
        .unwrap();
    ...
    let response: SimpleResponse = actor
        .invoke_method(
            "process_message",
            SimpleMessage {
    ...
    // Test state queries
    let _state: i32 = actor
        .invoke_method("get_state", ())
        .await
        .map_err(|e| anyhow::anyhow!(e))?;
    // Just verify we got a response - mock should provide a reasonable value

    let _count: u64 = actor
        .invoke_method("get_message_count", ())
        .await
        .map_err(|e| anyhow::anyhow!(e))?;
...
async fn test_mock_with_failure_simulation() -> anyhow::Result<()> {
    ...
    let result: Result<SimpleResponse, String> = actor
        .invoke_method(
            "process_message",
            SimpleMessage {
