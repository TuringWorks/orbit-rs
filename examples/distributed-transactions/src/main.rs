...
impl BankService {
    ...
    pub async fn create_account(&self, account_id: String, account_holder: String, initial_balance: f64) -> OrbitResult<()> {
    ...
    async fn validate_operations(&self, operations: &[TransactionOperation]) -> OrbitResult<bool> {
        ...
        for operation in operations {
            let banking_op: BankingOperation = serde_json::from_value(operation.operation_data.clone())
            ...
            match banking_op {
                BankingOperation::Debit { amount } => {
                    if !account.can_debit(amount) {
                        warn!("Cannot debit ${} from account {}: insufficient funds or frozen", amount, account_id);
                        return Ok(false);
                    }
                },
                BankingOperation::ReserveFunds { amount } => {
                    if !account.can_reserve(amount) {
                        warn!("Cannot reserve ${} in account {}: insufficient funds or frozen", amount, account_id);
                        return Ok(false);
                    }
                },
                BankingOperation::Transfer { amount, .. } => {
                    if !account.can_debit(amount) {
                        warn!("Cannot transfer ${} from account {}: insufficient funds or frozen", amount, account_id);
                        return Ok(false);
                    }
                },
    ...
    async fn execute_operations(&self, transaction_id: &TransactionId) -> OrbitResult<()> {
        ...
        info!("Executed {} banking operations for transaction {}", operations_count, transaction_id);
        Ok(())
    }
}
...
impl TransactionParticipant for BankService {
    async fn prepare(&self, transaction_id: &TransactionId, operations: &[TransactionOperation]) -> OrbitResult<TransactionVote> {
        info!("Preparing transaction {} with {} operations", transaction_id, operations.len());
    ...
    async fn abort(&self, transaction_id: &TransactionId, reason: &str) -> OrbitResult<()> {
        ...
        info!("Cleaned up resources for aborted transaction {}", transaction_id);
        Ok(())
    }
...
impl MockTransactionMessageSender {
    ...
    pub async fn register_participant(&self, actor_ref: AddressableReference, service: Arc<BankService>) {
...
impl TransactionMessageSender for MockTransactionMessageSender {
    async fn send_message(&self, target: &AddressableReference, message: TransactionMessage) -> OrbitResult<()> {
        ...
        if let Some(service) = participants.get(target) {
            match message {
                TransactionMessage::Prepare { transaction_id, operations, .. } => {
                ...
                TransactionMessage::Commit { transaction_id } => {
                    ...
                    info!("Participant {} committed transaction {}", target, transaction_id);
                },
                TransactionMessage::Abort { transaction_id, reason } => {
                    service.abort(&transaction_id, &reason).await?;
                    info!("Participant {} aborted transaction {}: {}", target, transaction_id, reason);
                },
                _ => {
...
async fn run_banking_transaction_example() -> OrbitResult<()> {
    ...
    // Create accounts
    bank_service1.create_account("alice".to_string(), "Alice Smith".to_string(), 1000.0).await?;
    bank_service1.create_account("bob".to_string(), "Bob Johnson".to_string(), 500.0).await?;
    bank_service2.create_account("charlie".to_string(), "Charlie Brown".to_string(), 750.0).await?;
    ...
    let charlie_ref = AddressableReference {
        addressable_type: "BankAccount".to_string(),
        key: Key::StringKey { key: "charlie".to_string() },
    };

    message_sender.register_participant(alice_ref.clone(), bank_service1.clone()).await;
    message_sender.register_participant(bob_ref.clone(), bank_service1.clone()).await;
    message_sender.register_participant(charlie_ref.clone(), bank_service2.clone()).await;
    ...
    ).with_compensation(serde_json::json!(BankingOperation::Credit { amount: 100.0 }));
    ...
    let tx_id2 = coordinator.begin_transaction(Some(Duration::from_secs(10))).await?;
    ...
    info!("Executing transaction {} (should fail due to insufficient funds)", tx_id2);
    // coordinator.commit_transaction(&tx_id2).await?;

    // Example 3: Complex multi-step transaction
    info!("\n=== Example 3: Complex multi-step transaction ===");
    
    let tx_id3 = coordinator.begin_transaction(Some(Duration::from_secs(15))).await?;
    ...
    info!("Executing complex transaction {} with fund reservation", tx_id3);
    ...
    info!("Alice: ${:.2} (reserved: ${:.2})", alice_final.balance, alice_final.reserved_funds);
