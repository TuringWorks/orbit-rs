...
use orbit_protocols::{
    ...
    vector_store::{Vector, VectorIndexConfig, SimilarityMetric, VectorActor, VectorStats}
};
...
async fn setup_vector_data(orbit_client: &orbit_client::OrbitClient) -> Result<(), Box<dyn std::error::Error>> {
    info!("📊 Setting up sample vector data...");
    
    // Get a VectorActor for our examples
    let vector_actor_ref = orbit_client.actor_reference::<VectorActor>(
        Key::StringKey { 
            key: "ml-embeddings".to_string() 
        }
    ).await.map_err(|e| format!("Failed to get actor reference: {}", e))?;
    ...
    let sample_vectors = vec![
        ...
        (
            ...
            HashMap::from([
                ("title".to_string(), "Deep Learning Fundamentals".to_string()),
                ("category".to_string(), "AI".to_string()),
        ...
        (
            ...
            HashMap::from([
                ("title".to_string(), "Computer Vision Applications".to_string()),
        ...
        (
            ...
            HashMap::from([
                ("title".to_string(), "Natural Language Processing".to_string()),
    ...
    for (id, data, metadata) in sample_vectors {
        ...
        vector_actor_ref.invoke("add_vector", vec![vector_value])
            .await.map_err(|e| format!("Failed to add vector {}: {}", id, e))?;
    ...
    let stats: VectorStats = vector_actor_ref.invoke("get_stats", vec![])
    ...
    info!("   • Dimension range: {} - {}", stats.min_dimension, stats.max_dimension);
...
fn print_usage_examples() {
    ...
    info!("   VECTOR.SEARCH ml-embeddings \"{}\" 3", 
          create_sample_vector(384, 0.15).iter()
              .take(10)
              .map(|f| format!("{:.3}", f))
              .collect::<Vec<_>>()
              .join(","));
